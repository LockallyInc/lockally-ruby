...
module Lockally
  class V1MailboxesEmailPatchRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::V1MailboxesEmailPatchRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::V1MailboxesEmailPatchRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@quota_bytes.nil? && @quota_bytes < 0
        invalid_properties.push('invalid value for "quota_bytes", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@quota_bytes.nil? && @quota_bytes < 0
      true
    end
    ...
    def quota_bytes=(quota_bytes)
      ...
      if quota_bytes < 0
        fail ArgumentError, 'invalid value for "quota_bytes", must be greater than or equal to 0.'
      end
    ...
    def hash
      [password, quota_bytes, disabled].hash
    end
