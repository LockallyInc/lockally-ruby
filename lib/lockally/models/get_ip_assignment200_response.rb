...
module Lockally
  class GetIPAssignment200Response < ApiModelBase
    attr_accessor :pool_name

    attr_accessor :pool_kind
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::GetIPAssignment200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::GetIPAssignment200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pool_kind')
        self.pool_kind = attributes[:'pool_kind']
      end
    ...
    def hash
      [pool_name, pool_kind, dedicated_ip, assigned_at, reason].hash
    end
