...
module Lockally
  class V1SendBatchPostRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::V1SendBatchPostRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::V1SendBatchPostRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @messages.length > 500
        invalid_properties.push('invalid value for "messages", number of items must be less than or equal to 500.')
      end
    ...
    def valid?
      ...
      return false if @messages.length > 500
    ...
    def messages=(messages)
      ...
      if messages.length > 500
        fail ArgumentError, 'invalid value for "messages", number of items must be less than or equal to 500.'
      end
