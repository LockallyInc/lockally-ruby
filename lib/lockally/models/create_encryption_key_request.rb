=begin
#lockally control plane

#The lockally control plane lets integrators and direct customers manage everything except actual mail data flow (which uses standard JMAP / IMAP / SMTP-submission against the data plane endpoints).  **Authentication.** All `/v1/*` endpoints require a Bearer API key in the `Authorization` header. Keys are formatted `lk_live_<8-char-prefix>_<32-char-secret>`. Generate the first key for a tenant via `cmd/seed`; subsequent keys via `POST /v1/api-keys`.  **Errors.** Failures return [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457) `application/problem+json` documents.  **Scopes.** Each endpoint requires a specific scope on the presented key. Insufficient scope returns `403` with the required scope name in `detail`. 

The version of the OpenAPI document: 0.1.0
Contact: support@lockally.com
Generated by: https://openapi-generator.tech
Generator version: 7.23.0

=end

require 'date'
require 'time'

module Lockally
  class CreateEncryptionKeyRequest < ApiModelBase
    attr_accessor :mailbox_email

    attr_accessor :public_key

    attr_accessor :encrypted_private_key

    attr_accessor :kdf_params

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'mailbox_email' => :'mailbox_email',
        :'public_key' => :'public_key',
        :'encrypted_private_key' => :'encrypted_private_key',
        :'kdf_params' => :'kdf_params'
      }
    end

    # Returns attribute mapping this model knows about
    def self.acceptable_attribute_map
      attribute_map
    end

    # Returns all the JSON keys this model knows about
    def self.acceptable_attributes
      acceptable_attribute_map.values
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'mailbox_email' => :'String',
        :'public_key' => :'String',
        :'encrypted_private_key' => :'String',
        :'kdf_params' => :'Object'
      }
    end

    # List of attributes with nullable: true
    def self.openapi_nullable
      Set.new([
      ])
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::CreateEncryptionKeyRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::CreateEncryptionKeyRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'mailbox_email')
        self.mailbox_email = attributes[:'mailbox_email']
      else
        self.mailbox_email = nil
      end

      if attributes.key?(:'public_key')
        self.public_key = attributes[:'public_key']
      else
        self.public_key = nil
      end

      if attributes.key?(:'encrypted_private_key')
        self.encrypted_private_key = attributes[:'encrypted_private_key']
      else
        self.encrypted_private_key = nil
      end

      if attributes.key?(:'kdf_params')
        self.kdf_params = attributes[:'kdf_params']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      warn '[DEPRECATED] the `list_invalid_properties` method is obsolete'
      invalid_properties = Array.new
      if @mailbox_email.nil?
        invalid_properties.push('invalid value for "mailbox_email", mailbox_email cannot be nil.')
      end

      if @public_key.nil?
        invalid_properties.push('invalid value for "public_key", public_key cannot be nil.')
      end

      if @encrypted_private_key.nil?
        invalid_properties.push('invalid value for "encrypted_private_key", encrypted_private_key cannot be nil.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      warn '[DEPRECATED] the `valid?` method is obsolete'
      return false if @mailbox_email.nil?
      return false if @public_key.nil?
      return false if @encrypted_private_key.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] mailbox_email Value to be assigned
    def mailbox_email=(mailbox_email)
      if mailbox_email.nil?
        fail ArgumentError, 'mailbox_email cannot be nil'
      end

      @mailbox_email = mailbox_email
    end

    # Custom attribute writer method with validation
    # @param [Object] public_key Value to be assigned
    def public_key=(public_key)
      if public_key.nil?
        fail ArgumentError, 'public_key cannot be nil'
      end

      @public_key = public_key
    end

    # Custom attribute writer method with validation
    # @param [Object] encrypted_private_key Value to be assigned
    def encrypted_private_key=(encrypted_private_key)
      if encrypted_private_key.nil?
        fail ArgumentError, 'encrypted_private_key cannot be nil'
      end

      @encrypted_private_key = encrypted_private_key
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          mailbox_email == o.mailbox_email &&
          public_key == o.public_key &&
          encrypted_private_key == o.encrypted_private_key &&
          kdf_params == o.kdf_params
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [mailbox_email, public_key, encrypted_private_key, kdf_params].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      attributes = attributes.transform_keys(&:to_sym)
      transformed_hash = {}
      openapi_types.each_pair do |key, type|
        if attributes.key?(attribute_map[key]) && attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = nil
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[attribute_map[key]].is_a?(Array)
            transformed_hash["#{key}"] = attributes[attribute_map[key]].map { |v| _deserialize($1, v) }
          end
        elsif !attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = _deserialize(type, attributes[attribute_map[key]])
        end
      end
      new(transformed_hash)
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = self.class.openapi_nullable.include?(attr)
          next if !is_nullable || (is_nullable && !instance_variable_defined?(:"@#{attr}"))
        end

        hash[param] = _to_hash(value)
      end
      hash
    end

  end

end
