...
module Lockally
  class GetCalendarSecurity200ResponseDelegatedAccessInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::GetCalendarSecurity200ResponseDelegatedAccessInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::GetCalendarSecurity200ResponseDelegatedAccessInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'calendar_name')
        self.calendar_name = attributes[:'calendar_name']
      end
    ...
    def hash
      [calendar_id, calendar_name, user_email, role, granted_at].hash
    end
