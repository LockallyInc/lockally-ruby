  ...
  class Webhook < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::Webhook` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::Webhook`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] paused Value to be assigned
    def paused=(paused)
      if paused.nil?
        fail ArgumentError, 'paused cannot be nil'
      end
    ...
    def hash
      [id, tenant_id, url, events, paused, paused_at, last_success_at, last_failure_at, consecutive_failures, created_at, signing_secret].hash
    end
