...
module Lockally
  class SharedMember < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::SharedMember` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::SharedMember`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] member_email Value to be assigned
    def member_email=(member_email)
      if member_email.nil?
        fail ArgumentError, 'member_email cannot be nil'
      end
    ...
    def hash
      [id, mailbox_id, tenant_id, member_email, role, created_at].hash
    end
