...
module Lockally
  class GetCalendarSecurity200ResponseAlertsInner < ApiModelBase
    attr_accessor :level
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::GetCalendarSecurity200ResponseAlertsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::GetCalendarSecurity200ResponseAlertsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      level_validator = EnumAttributeValidator.new('String', ["critical", "warning"])
    ...
    def level=(level)
      validator = EnumAttributeValidator.new('String', ["critical", "warning"])
