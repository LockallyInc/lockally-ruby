...
module Lockally
  class CalendarIntegration < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::CalendarIntegration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::CalendarIntegration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      provider_validator = EnumAttributeValidator.new('String', ["exchange", "google_calendar", "teams", "zoom", "erp"])
      return false unless provider_validator.valid?(@provider)
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('String', ["connected", "disconnected", "error", "syncing"])
    ...
    def provider=(provider)
      validator = EnumAttributeValidator.new('String', ["exchange", "google_calendar", "teams", "zoom", "erp"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["connected", "disconnected", "error", "syncing"])
    ...
    def hash
      [id, tenant_id, provider, label, status, last_sync_at, error_message, created_at, updated_at].hash
    end
