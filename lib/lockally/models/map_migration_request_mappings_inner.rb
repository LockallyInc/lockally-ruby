...
module Lockally
  class MapMigrationRequestMappingsInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::MapMigrationRequestMappingsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::MapMigrationRequestMappingsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @dest_email.nil?
        invalid_properties.push('invalid value for "dest_email", dest_email cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @dest_email.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] dest_email Value to be assigned
    def dest_email=(dest_email)
      if dest_email.nil?
        fail ArgumentError, 'dest_email cannot be nil'
      end
