...
module Lockally
  class MigrationCredential < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Lockally::MigrationCredential` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Lockally::MigrationCredential`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      provider_validator = EnumAttributeValidator.new('String', ["imap", "google", "microsoft", "generic"])
    ...
    def provider=(provider)
      validator = EnumAttributeValidator.new('String', ["imap", "google", "microsoft", "generic"])
    ...
    def hash
      [id, tenant_id, provider, encryption_key_id, label, created_at, updated_at].hash
    end
